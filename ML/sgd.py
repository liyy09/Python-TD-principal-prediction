...
best_results_df = pd.DataFrame(columns=['ahead', 'learning_rate', 'eta0', 'mae', 'rmse', 'mape'])
all_results_df = pd.DataFrame(columns=['ahead', 'learning_rate', 'eta0', 'mae', 'rmse', 'mape'])
...
for ahead in range(start, end):
    ...
    for learning_rate in learning_rate_strategies:
        for eta0 in eta0_values:
            try:
                ...
                print(f"Failed: LR={learning_rate}, eta0={eta0}")
    ...
    if results:
        ...
        for i, result in enumerate(sorted_results, 1):
            print(f"{i}. LR={result['learning_rate']}, Eta0={result['eta0']}, "
        ...
        # plt.title(f'SGD Regressor: True vs Predicted Values (Ahead={ahead})\n'
        #           f'Learning Rate={best_result["learning_rate"]}, Eta0={best_result["eta0"]}')
        # plt.xlabel('Sample Index')
        # plt.ylabel('sqale_index')
        # plt.legend()
        # plt.grid(True, alpha=0.3)
        #
        # plot_path = os.path.join(desktop_path, f"sgd_prediction_ahead_{ahead}.png")
...
best_results_path = os.path.join(desktop_path, "sgd_best_results.csv")
all_results_path = os.path.join(desktop_path, "sgd_all_results.csv")
