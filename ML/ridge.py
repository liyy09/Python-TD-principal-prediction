...
for ahead in range(start, end):
    ...
    # plt.title(f'Ridge Regression: True vs Predicted Values (Ahead={ahead}, Alpha={best_result["alpha"]})')
    # plt.xlabel('Sample Index')
    # plt.ylabel('sqale_index')
    # plt.legend()
    # plt.grid(True, alpha=0.3)
    #
    # plot_path = os.path.join(desktop_path, f"ridge_prediction_ahead_{ahead}.png")
    # plt.savefig(plot_path, dpi=300, bbox_inches='tight')
    # plt.show()
    #
    # print(f"The prediction results image has been saved to: {plot_path}")

best_results_path = os.path.join(desktop_path, "l2_best_results.csv")
all_results_path = os.path.join(desktop_path, "l2_all_results.csv")
