...
best_results_df = pd.DataFrame(columns=['ahead', 'C', 'epsilon', 'mae', 'rmse', 'mape'])
all_results_df = pd.DataFrame(columns=['ahead', 'C', 'epsilon', 'mae', 'rmse', 'mape'])
...
# Record the results of all parameter combinations
    for C in C_values:
        for epsilon in epsilon_values:
            try:
                ...
                print(f"Failed: C={C}, epsilon={epsilon}")
    ...
    if results:
        ...
        for i, result in enumerate(sorted_results, 1):
            print(f"{i}. C={result['C']}, epsilon={result['epsilon']}, "
                  f"MAE={result['mae']:.3f}, RMSE={result['rmse']:.3f}, MAPE={result['mape']:.3f}")

        # # Plotting predicted values ​​and actual valuesults线图
        ...
        # plt.title(f'SVR (Linear): True vs Predicted Values (Ahead={ahead})\n'
        #           f'C={best_result["C"]}, epsilon={best_result["epsilon"]}')
        ...
        # plot_path = os.path.join(desktop_path, f"svr_linear_prediction_ahead_{ahead}.png")
...
# 保存结果到CSV文件
best_results_path = os.path.join(desktop_path, "svr_linear_best_results.csv")
all_results_path = os.path.join(desktop_path, "svr_linear_all_results.csv")

best_results_df.to_csv(best_results_path, index=False)
all_results_df.to_csv(all_results_path, index=False)

print(f"\nBest result have been saved: {best_results_path}")
print(f"Best result have been saved: {all_results_path}")