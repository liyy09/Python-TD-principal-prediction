...
best_results_df = pd.DataFrame(columns=['ahead', 'n_estimators', 'max_depth', 'max_features', 'mae', 'rmse', 'mape'])
all_results_df = pd.DataFrame(columns=['ahead', 'n_estimators', 'max_depth', 'max_features', 'mae', 'rmse', 'mape'])
...
for ahead in range(start, end):
        ...
        for max_depth in max_depth_values:
            for max_features in max_features_values:
                try:
                    ...
                    print(f"Failed: n_estimators={n_estimators}, max_depth={max_depth}, "
    ...
    if results:
        ...
        print(f"\nTop 5 best results:")
        sorted_results = sorted(results, key=lambda x: x['mape'])[:5]
        for i, result in enumerate(sorted_results, 1):
            print(f"{i}. n_est={result['n_estimators']}, max_d={result['max_depth']}, "
                  f"max_feat={result['max_features']}, MAE={result['mae']:.3f}, RMSE={result['rmse']:.3f}, MAPE={result['mape']:.3f}")
    ...
    # # Plotting predicted values ​​and actual value
        ...
        # plt.title(f'Random Forest: True vs Predicted Values (Ahead={ahead})\n'
        #           f'n_estimators={best_result["n_estimators"]}, max_depth={best_result["max_depth"]}, max_features={best_result["max_features"]}')
        # plt.xlabel('Sample Index')
        # plt.ylabel('sqale_index')
        # plt.legend()
        # plt.grid(True, alpha=0.3)
        #
        # plot_path = os.path.join(desktop_path, f"rf_prediction_ahead_{ahead}.png")
...
best_results_path = os.path.join(desktop_path, "rf_best_results.csv")
all_results_path = os.path.join(desktop_path, "rf_all_results.csv")
